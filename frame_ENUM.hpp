...
class Table {
  ...
  public:
    ...
    void setFuture(int index, state value) {
      future[index] = value;
    }

    state getCellValue(int i) {
      return current[i];
    }
    ...
    vector<state> getNeighbours(int i) {
      long column, row;
      row = i / width;
      column = i % width;
...
class Game {
  ...
  public:
    ...
    void execute(int start, int stop) {
      for (int j = 0; j < nSteps; j++) {
        for (int i = start; i <= stop; i++) {
          state val = table.getCellValue(i);
          state nVal = rule(val, table.getNeighbours(i));
    ...
    virtual state rule(state val, vector<state> arr) { return zero; };
